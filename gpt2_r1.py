...
def load_data():
    ...
    numerical_column_names = ['Digital', 'DigitalOOH', 'OOH', 'Print', 'Radio', 'SEM', 'SEM 2', 'Social', 'TV', 'Awareness']
    ...
    for idx, row in iTrade_dataset_df.iterrows():
        ...
        for i in range(num_iTrade_htmls):
            for j in range(4):
                column_name = 'Text %d-%d'%(i+1, j+1)
                if column_name in iTrade_columns:
                    texts.append(row[column_name] if not is_nan(row[column_name]) else "")
