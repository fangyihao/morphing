...
attr_method = 'gausslegendre' # 'gausslegendre' 'riemann_left' 'riemann_right' 'riemann_middle' 'riemann_trapezoid'
baseline_method = 'gaussblur' # 'pad'
spending_percentiles = [0.2,0.4,0.6,0.8] #[0.14286, 0.28571, 0.42857, 0.57143, 0.71429, 0.85714]  [0.3333,0.6667]  [0.2,0.4,0.6,0.8]
funnel_percentiles = [0.3333,0.6667]
smooth_grad = False
attr_batch_size = 8
attr_steps = 300
attr_prompts = False
...
attr_delta_valid_range = [-0.190023642, 0.28805191]
...
def _load_data(business_unit):
    ...
    def calculate_percentiles(dataset_df):
        ...
        spending_percentile_df = dataset_df[spending_column_names].quantile([0] + spending_percentiles + [1])
        #spending_percentile_df["Percentile"] = [0] + spending_percentiles + [1]
        funnel_percentile_df = dataset_df[funnel_column_names].quantile([0] + funnel_percentiles + [1])
    ...
    with codecs.open('spending_percentiles.csv', 'w', 'utf-8') as csv_file:
        spending_percentile_df.to_csv(csv_file, index=True, lineterminator='\n')
        
    with codecs.open('funnel_percentiles.csv', 'w', 'utf-8') as csv_file:
    ...
    #plt.show()
    plt.savefig("%s_Labels.png"%business_unit)
    plt.clf()
    ...
    #plt.show()
    plt.savefig("%s_Document_Lengths.png"%business_unit)
    plt.clf()
    
    labels = labels[num_weeks-1:]
    label_attrs = label_attrs[num_weeks-1:]
...
def load_data():
    ...
    for business_unit in business_units:
        doc_seqs, labels, label_attrs = _load_data(business_unit)
...
class Dataset(Dataset):
    ...
    def __init__(self, source, target, target_attrs, tokenizer, max_length=768):
...
def test(model, tokenizer):
    ...
    funnel_percentile_df = pd.read_csv('funnel_percentiles.csv', index_col=0)
    print(funnel_percentile_df)
    ...
    for batch in test_dataloader:
        ...
        b_df = pd.DataFrame({'Week': b_label_attrs[1],
                    'Label': b_labels,
                   'Label Real': b_label_attrs[0].detach().tolist(),
                   'Prediction': preds, 
                   'Prediction Approx': [scale2value(pred, funnel_percentile_df['Purchase over Consideration'].values) for pred in preds]})
    ...
    print(df)
    with codecs.open('test_preds.csv', 'w', 'utf-8') as csv_file:
...
def attribute(model, tokenizer):
            ...
            if model_type.startswith("t5") or model_type.startswith("google/t5"):
                ...
                for i in range(batch_size):
                    ...
                    attributions, delta = ig.attribute(inputs=b_input_ids[i].unsqueeze(0),
                                  ...
                                  return_convergence_delta = True, internal_batch_size = attr_batch_size, n_steps = attr_steps, smooth_grad = smooth_grad)
                    ...
                    if delta.item() >= attr_delta_valid_range[0] and delta.item() < attr_delta_valid_range[1] and torch.argmax(scores).item() == b_labels[i][0].item():
    ...
    with codecs.open('%s_ig_%s.html'%('word_importance' if attr_decode_subwords else 'subword_importance',attr_method), 'w', 'utf-8') as file:
        file.write(importance_html_data)
        
    with codecs.open('%s_ig_%s.csv'%('word_importance' if attr_decode_subwords else 'subword_importance',attr_method), 'w', 'utf-8') as file:
    ...
    writer = pd.ExcelWriter('%s_n_attr.xlsx'%('word' if attr_decode_subwords else 'subword'), engine='xlsxwriter')
    for sheet_name in sheet_names:
        df = pd.DataFrame(np.array(word_n_attr_excel_data[sheet_name]), columns=["Word" if attr_decode_subwords else "Subword", "Attribution Score", "Attribution Scale"])
...
if __name__ == '__main__':
    ...
    doc_seqs, labels, label_attrs = load_data()
    tokenizer = load_tokenizer()
    if model_type.startswith("gpt2"):
        dataset = Dataset(doc_seqs, labels, label_attrs, tokenizer, max_length=seq_len)
    else:
        dataset = Dataset(doc_seqs, labels, label_attrs, tokenizer, max_length=seq_len)
